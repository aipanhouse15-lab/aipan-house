...
export function generateMetadata({ params }) {
  const a = getBySlug('festivals', params.slug)
  if (!a) return {}
  return {
    title: a.title,
...
export default function FestivalsArticle({ params }) {
  const a = getBySlug('festivals', params.slug)
  if (!a) notFound()
  return (
    <article className="mx-auto max-w-3xl px-6 py-14">
      <Link href="/festivals" className="font-sans text-sm text-geru hover:text-geru-deep">← Festivals</Link>
